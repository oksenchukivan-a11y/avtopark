...
const APP_VERSION = 'v88';          // показуємо в шапці — щоб видно було, що отримав свіже
...
function calFuel(raw, md) {
  ...
  // ЖОДНИХ «стель» і порогів: будь-який поріг створює ОБРИВ на межі — 19.08 датчик Master перейшов
  // через таку межу і застосунок нарахував 26 л замість 7 (розхід 36 л/100 на рівному місці).
  // Калібрування ЛИШЕ лінійне: множник (+ зсув, якщо шкала зміщена). Перевіряти по чеках заправок.
  ...
  try {
    ...
    for (const m of (resA || [])) {
      ...
      if (la != null && lo != null && saneRegion(la, lo) && m['position.valid'] !== false && (sa == null || sa >= 3)) {
        anchor = [la, lo, m.timestamp]; break;
      }
    }
